...
export default function CalendarScreen() {
  ...
  const addTimeEntry = async () => {
    if (clients.length === 0) {
      Alert.alert(
        "No Clients",
        "Please add a client first before tracking time.",
      );
      return;
    }

    Alert.alert("Add Time Entry", "Select duration for today", [
      {
        text: "Cancel",
        style: "cancel",
      },
  ...
  const selectClient = (durationType: "full_day" | "half_day") => {
    ...
    Alert.alert("Select Client", "Which client did you work for?", [
  ...
  const deleteTimeEntry = async (entryId: string) => {
    try {
      await powersync.execute("DELETE FROM time_entries WHERE id = ?", [
        entryId,
      ]);
      loadTimeEntries();
    } catch (error) {
  ...
  const renderTimeEntry = ({ item }: { item: TimeEntry }) => {
    const earnings = item.client
      ? item.duration_type === "full_day"
        ? item.client.daily_rate
        : item.client.daily_rate / 2
      : 0;
